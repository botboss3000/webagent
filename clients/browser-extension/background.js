...
chrome.runtime.onMessage.addListener((m, sender, sendResponse) => {
  ...
  if (m.type === "set_pause") { cfg.paused = !!m.paused; saveCfg(); updateBadge(); sendResponse(snapshot()); return true; }
  ...
  if (m.type === "config_changed") { loadCfg().then(() => { if (cfg.autoConnect) connect(); updateBadge(); sendResponse(snapshot()); }); return true; }
  return false;
});
