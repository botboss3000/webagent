...
@dataclass
class ChargeResult:
    """What the user pays, what the platform takes, what the agent admin earns."""
    end_user_charge_cents: int = 0
    platform_fee_cents: int = 0
    agent_admin_earnings_cents: int = 0
    strategy: str = "free"
...
def _merge_configs(platform: dict, agent: dict) -> dict:
    """Agent config overrides platform config field-by-field.

    Strategy/processors on the agent row must already have been filtered to
    the platform's allowed set by the API (we don't re-filter here)."""
    if not agent:
        return platform
    out = dict(platform)
    for k, v in agent.items():
        if v in (None, "", [], {}, 0) and k not in (
            "platform_fee_pct", "platform_fee_flat_cents",
            "subscription_price_cents",
        ):
            # Treat zero-ish values as "inherit from platform" for non-numeric
            # fields. Numeric fee fields explicitly inherit on zero too — agent
            # admins use the agent row's strategy + rate card, the platform fee
            # always comes from the platform row.
            continue
        out[k] = v
    # Platform fee always comes from the platform row.
    out["platform_fee_pct"] = platform.get("platform_fee_pct", 0)
    out["platform_fee_flat_cents"] = platform.get("platform_fee_flat_cents", 0)
    out["allowed_strategies"] = platform.get("allowed_strategies", []) or []
    out["allowed_processors"] = (
        agent.get("allowed_processors")
        if agent.get("allowed_processors")
        else platform.get("allowed_processors", [])
    )
    return out
...
async def load_effective_config(db: Any, agent_id: str) -> dict:
    """Return platform-config merged with agent-scoped overrides.

    Falls back to an all-defaults 'free' config when nothing is set, so
    ...
    platform = _row_to_config(platform_row) if platform_row else _empty_config()
    agent = _row_to_config(agent_row) if agent_row else {}
    return _merge_configs(platform, agent)
...
def _split_fee(end_user_amount: int, cfg: dict) -> tuple[int, int]:
    """Return (platform_fee_cents, agent_admin_earnings_cents)."""
    if end_user_amount <= 0:
        return 0, 0
    pct = float(cfg.get("platform_fee_pct") or 0)
    flat = int(cfg.get("platform_fee_flat_cents") or 0)
    fee = int(round(end_user_amount * pct / 100.0)) + flat
    fee = max(0, min(fee, end_user_amount))
    earnings = end_user_amount - fee
    return fee, earnings
...
async def resolve_charge(
    ...
    *,
    ...
    if strategy in _STRATEGY_DISPATCH:
        ...
        fee, earnings = _split_fee(end_user, cfg)
        return ChargeResult(
            end_user_charge_cents=end_user,
            platform_fee_cents=fee,
            agent_admin_earnings_cents=earnings,
            strategy=strategy,
            is_byo_llm=byo,
        )
