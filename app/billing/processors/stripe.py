...
class StripeProcessor:
    ...
    async def refund(self, *, external_payment_id, amount_cents) -> Refund:
        if not self.is_configured():
            raise RuntimeError("Stripe is not configured")
        stripe = self._stripe
        r = stripe.Refund.create(payment_intent=external_payment_id, amount=int(amount_cents))
        return Refund(refund_id=r.id, amount_cents=int(amount_cents), processor=self.name, status=r.status)
