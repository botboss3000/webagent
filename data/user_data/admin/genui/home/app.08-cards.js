...
function activate(id){if(id!==STATE.activeId){STATE.openItem=null;STATE.itemQ='';STATE.itemSearchOpen=false}STATE.activeId=id;if(!id)STATE.openItem=null;savePageState();var i,els=document.querySelectorAll('.card');for(i=0;i<els.length;i++)els[i].classList.toggle('active',els[i].dataset.id===id);renderDetail()}
