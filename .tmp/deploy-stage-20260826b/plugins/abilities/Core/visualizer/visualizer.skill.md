...
| `present_chat_component(type, title, placement, data)` | Add a safe, declarative component to the current chat. Use `placement:"inline"` for one-shot contextual UI, `"sticky"` for a persistent session panel, or `"hover"` for compact secondary UI. **Title = 1–2 words, max 3** (e.g. `"Checklist"`, `"Plan Overview"`) — never a sentence; long titles are capped server-side to the element type name. Never use it to collect a password, API key, or other secret — use `request_credential` instead. |
...
### Chat elements: naming & icons (COMPONENT-TYPE-ICON-STANDARD)

Chat elements rendered by `present_chat_component` / `present_plan` follow two fixed conventions:

- **Titles are terse: 1–2 words, max 3.** Name the element, not the task — `"Checklist"`, `"Plan Overview"`, `"Approval"`, `"Choice"`, `"Form"`. Never `"Persist agent-panel UI state per session Checklist"`. The server caps titles at 3 words and falls back to the element type name (`Checklist`, `Overview`, `Choice`, `Approval`, `Form`), so even a long title ends up short — but pass a short one in the first place.
- **Icons are fixed per type — the app picks them, you don't.** `todo_list`/Checklist → check-in-box (`check-square`); `status`/Overview & plans → `square-text` (always); `choice`, `approval`, `form` (elements with buttons) → `square-square`. The same icon shows in the panel head and the session dropdown chips. Don't try to pass or override an icon.
