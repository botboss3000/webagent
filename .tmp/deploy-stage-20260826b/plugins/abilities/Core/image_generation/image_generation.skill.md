...
## Taking over — `set_model` (needs the Model Switcher ability)

If a stretch of the conversation is image-generation heavy and a model exists that
can be the **brain** (text **and** tools) **and** output images, you may run on it
directly — but that's the separate **Model Switcher** ability: `set_model('<model>')`.
If you don't have that ability, or only image-only output models are configured,
taking over isn't possible — that's expected; **keep using `generate_image`** to
delegate. Revert with `set_model('default')` when done.
...
- Heavy generation stretch, a text+image-out model exists, *and* you have the Model
  Switcher ability → optional `set_model('<model>')`, then revert with
  `set_model('default')`.
