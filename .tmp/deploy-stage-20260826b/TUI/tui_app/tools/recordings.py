...
that captures one chat baseline, frame-grouped DOM differences, lag / long-task
metrics, JS errors, console warnings, and failed/slow network calls from the live
web UI.
...
here — and turn it off again when you're done. Each enable cycle records for at
most 30 seconds, with capture id and elapsed milliseconds stored in each row's
detail.
"""
