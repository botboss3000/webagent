...
class LLMClient:
    ...
    @staticmethod
    def _format_context_overflow(data: dict[str, Any], model: str) -> Optional[str]:
        """If ``data`` is an OpenRouter context-overflow error, return a clear
        explanation; otherwise None. Never retries — this is a hard cap."""
        if not isinstance(data, dict):
            return None
        msg = data.get("message") or ""
        if not ("max_num_tokens" in msg and "prompt length" in msg):
            return None
        import re
        prompt_match = re.search(r"prompt length\s*\(([0-9.]+)\)", msg)
        max_match = re.search(r"max_num_tokens\s*\(([0-9.]+)\)", msg)
        prompt_tokens = int(float(prompt_match.group(1))) if prompt_match else None
        max_tokens = int(float(max_match.group(1))) if max_match else None
        parts = ["Context window exceeded."]
        if model:
            parts.append(f"Model: {model}" + ("." if not (prompt_tokens and max_tokens) else ""))
        if prompt_tokens is not None and max_tokens is not None:
            parts.append(
                f"The prompt is {prompt_tokens:,} tokens but this model only "
                f"accepts {max_tokens:,}."
            )
        parts.append(
            "Start a fresh session with /new, or delete old turns to free space. "
            "Your full conversation is preserved on disk — nothing is lost."
        )
        return " ".join(parts)
    ...
    async def _attempt(self, body: dict[str, Any], host: str) -> Completion:
        ...
        # Parse the body first so we can detect context-overflow errors in both
        # HTTP 400 and 200-with-error responses.
        try:
            data = resp.json()
        except json.JSONDecodeError as e:
            if resp.status_code >= 400:
                raise LLMError(
                    f"HTTP {resp.status_code} from {host}: {resp.text[:400]!r}"
                ) from e
            raise _TransientLLMError(
                f"{host} returned non-JSON (HTTP {resp.status_code}): {resp.text[:400]!r}"
            ) from e
        # ── Context-overflow detection (hard limit — never retry) ──────────
        ctx_err = self._format_context_overflow(data, body.get("model", ""))
        if ctx_err:
            raise LLMError(ctx_err)
        if resp.status_code >= 400:
