...
class ServerManagerAgent:
    def __init__(
        ...
        self.drain_steer: Optional[Callable[[str], Optional[str]]] = None
    ...
    def _build_messages(self, session_id: str, situation: str = "") -> list[dict[str, Any]]:
        ...
        # Per-session prompt override (e.g. a dedicated Watchdog prompt).
        session_prompt = self.store.get_session_prompt(session_id)
        if session_prompt:
    ...
    def _absorb_external(self, session_id: str) -> bool:
        ...
        **steering** messages the user typed mid-turn are appended verbatim. Both
        ...
        if self.subagents is not None:
            drained = self.subagents.drain_surfaced(session_id)
            if drained:
        ...
        if self.drain_steer is not None:
            while True:
                steer = self.drain_steer(session_id)
                if not steer:
                    break
    ...
    def _has_surfaced_results(self, session_id: str) -> bool:
        return self.subagents is not None and self.subagents.has_surfaced_pending(session_id)
    ...
    async def run_turn(
        ...
        for _turn in range(self.cfg.max_turns):
            ...
            if not comp.tool_calls:
                ...
                if self._has_surfaced_results(session_id):
                    continue
