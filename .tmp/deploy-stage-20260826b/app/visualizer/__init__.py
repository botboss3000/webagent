...
def register_tools(tools: Dict[str, ToolInfo], user_id: str, agent_id: str = "") -> None:
    ...
    # The server persists this validated, declarative payload against the run's
    # chat session right here, so the component survives even when no browser is
    # connected to receive the live tool_result (a reload hydrates it via the
    # GET /chat/components load path). The frontend renders from the streamed
    # result; it no longer POSTs the component itself. No agent-authored HTML or
    # JS is ever allowed to run inside the chat shell.
    async def _present_chat_component(type: str, title: str = "", placement: str = "inline",
        ...
        from app.chat_components import validate_spec, save_component
        ...
        # Durable server-side save (same row shape the browser used to POST).
        # Best-effort: a save failure must not fail the tool — the live event
        # would still render the component for connected clients.
        try:
            ctx = current_tool_context()
            if ctx is not None:
                await save_component(ctx.user_id, ctx.session_id, component)
        except Exception:
            pass
    ...
    tools["present_chat_component"] = ToolInfo(
        ...
        parameters={"type": "object", "properties": {
            ...
            "title": {"type": "string",
                      "description": "Short display name — 1-2 words, max 3 (e.g. 'Checklist', 'Plan Overview'). Long names are capped server-side to the element type name."},
    ...
    # -- present_plan -----------------------------------------------------------
    # The plan-mode workspace writer. One call updates BOTH halves of the plan
    # (overview + checklist) with the stable component ids the planning directive
    # names, so an agent can never drift on naming and updates replace cleanly.
    # The overview is a status component (goal / criteria / approach / notes
    # sections), the checklist a todo_list of milestones. Fulfilled checklists
    # drop out of the active set via the normal completion lifecycle.
    async def _present_plan(overview: dict = None, checklist: list = None,
                            goal: str = "", criteria: str = "", notes: str = "",
                            title: str = "Plan", **_extra):
        from app.chat_components import validate_spec, save_component, short_title
        from app.tools.execution_context import current_tool_context
        ctx = current_tool_context()
        if ctx is None:
            return json.dumps({"status": "error", "message": "no active session context"})
        # Component titles stay terse (1-2 words, max 3). A short plan title
        # prefixes the element name ("Plan Overview"); a long one is dropped so
        # the components are just "Overview" / "Checklist".
        words = [w for w in str(title or "").split() if w]
        if len(words) <= 2:
            base = " ".join(words) or "Plan"
            overview_title = short_title(base + " Overview", "Overview")
            checklist_title = short_title(base + " Checklist", "Checklist")
        else:
            overview_title = "Overview"
            checklist_title = "Checklist"
        sections = []
        if goal:
            sections.append({"label": "Goal", "status": None, "value": goal})
        if criteria:
            sections.append({"label": "Criteria", "status": None, "value": criteria})
        for s in (overview or {}).get("sections", []):
            if s not in sections:
                sections.append(s)
        if notes:
            sections.append({"label": "Notes", "status": None, "value": notes})
        # The overview renders as a MARKDOWN document (the status renderer uses
        # data.markdown when present; sections are the structured fallback).
        md_parts = []
        for s in sections:
            md_parts.append(f"## {s['label']}")
            if s.get("status"):
                md_parts.append(f"*{s['status']}*")
            if s.get("value"):
                md_parts.append(s["value"])
        overview_comp = validate_spec({
            "id": "plan-overview", "type": "status", "title": overview_title,
            "placement": "sticky",
            "data": {"sections": sections, "markdown": "\n\n".join(md_parts)},
            "lifecycle": {}})
        checklist_comp = validate_spec({
            "id": "plan-checklist", "type": "todo_list", "title": checklist_title,
            "placement": "sticky",
            "data": {"items": checklist or [{"id": "p1", "label": "Draft plan", "done": False}]},
            "lifecycle": {}})
        try:
            await save_component(ctx.user_id, ctx.session_id, overview_comp)
            await save_component(ctx.user_id, ctx.session_id, checklist_comp)
        except Exception as exc:
            return json.dumps({"status": "error", "message": str(exc)})
        return json.dumps({"status": "ok", "ui": "chat_component",
                           "components": [overview_comp, checklist_comp]})

    tools["present_plan"] = ToolInfo(
        name="present_plan", handler=_present_plan,
        parameters={"type": "object", "properties": {
            "overview": {"type": "object",
                         "description": "Optional overview sections: {sections: [{label, status, value}]}"},
            "checklist": {"type": "array", "items": {"type": "object"},
                          "description": "Checklist milestones: [{id, label, done}]"},
            "goal": {"type": "string"}, "criteria": {"type": "string"},
            "notes": {"type": "string"},
            "title": {"type": "string",
                      "description": "Short plan name — 1-2 words, max 3 (default 'Plan'). The plan components are named '&lt;title&gt; Overview' / '&lt;title&gt; Checklist' when the title is short; a long title is dropped and they become just 'Overview' / 'Checklist'."},
        }},
    )
