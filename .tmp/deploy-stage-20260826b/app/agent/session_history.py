...
_OFF_TASK_INPUT_REASON = "completed earlier task"
...
def interactions_to_openai_messages(
    ...
    *,
    ...
    _hide_off_task = off_task_hiding_enabled()
    ...
    def _is_off_task(row: InteractionRecord) -> bool:
        t = _task_of.get(row.id)
        return _hide_off_task and t is not None and t != _current_task
    ...
    def _compact_tool_calls(
        ...
        if not _is_off_task(assistant_row):
            return calls
    ...
    def _tool_content(tr: InteractionRecord) -> str:
        if _is_off_task(tr):
            return _OFF_TASK_TOOL_PLACEHOLDER.format(
