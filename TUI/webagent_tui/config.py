...
def data_dir() -> Path:
    ...
    This is the **mk2 (event-driven) build**, so it uses its OWN data dir
    (``webagent-tui-mk2``) — keeping its sessions, config, and subagent
    transcripts entirely separate from the original ``webagent-tui`` build, which
    is left untouched as a fallback.

    Windows: %APPDATA%\\webagent-tui-mk2   macOS: ~/Library/Application Support/...
    Linux:   $XDG_DATA_HOME/webagent-tui-mk2 or ~/.local/share/webagent-tui-mk2
    """
    if sys.platform == "win32":
        base = os.environ.get("APPDATA") or str(Path.home() / "AppData" / "Roaming")
        return Path(base) / "webagent-tui-mk2"
    if sys.platform == "darwin":
        return Path.home() / "Library" / "Application Support" / "webagent-tui-mk2"
    xdg = os.environ.get("XDG_DATA_HOME")
    base = Path(xdg) if xdg else Path.home() / ".local" / "share"
    return base / "webagent-tui-mk2"
