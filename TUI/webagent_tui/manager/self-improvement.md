...
## Where code lives (relative to `webagent_tui/`)

| File/Dir | Purpose |
|---|---|
| `__main__.py` | Entry point — `python -m webagent_tui` → `app.py:run()` |
| `app.py` | Textual TUI (chat screen, panels, user settings, server start/stop UI) |
| `agent.py` | `ServerManagerAgent` — builds system prompt + history, calls LLM, dispatches tools |
...
| `manager/prompt.md` | **THIS file** — loaded at startup; restart to apply changes |
| `manager/tools.json` | Human-readable tool descriptions + enable flags overlaying Python specs |
| `manager/monitor.defaults.json` | Shipped watchdog defaults |
...
- `resources.py`: user override (`data_dir()/prompt.md`) → packaged default (`manager/prompt.md`) → built-in fallback string
...
3. **Optionally** add a description/enable flag in `manager/tools.json`
...
- **Windows**: `%APPDATA%\webagent-tui`
- **macOS**: `~/Library/Application Support/webagent-tui`
...
| TUI DB (chat history, audit) | `data_dir() / webagent_tui.db` |
...
1. `WEBAGENT_TUI_API_KEY` / `WEBAGENT_TUI_BASE_URL` / `WEBAGENT_TUI_MODEL` env vars
