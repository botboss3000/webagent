...
* **source** — launched as ``python -m webagent_tui`` from a checkout (``run.bat``).
  Its code IS the ``webagent_tui/`` package inside the public webAgent git repo,
...
def package_dir() -> Path:
    """The installed ``webagent_tui`` package directory."""
...
def _build_stamp() -> tuple[str, str]:
    ...
    ``build_exe.py`` writes ``webagent_tui/_build.py`` just before bundling, so a
...
def tui_project_dir() -> Optional[Path]:
    """The ``webagent_tui/`` project dir (the one with ``pyproject.toml`` +
    ``scripts/``). Source mode only."""
    if is_frozen():
        return None
    cand = package_dir().parent          # .../webagent_tui (project) / webagent_tui (package)
...
class SelfInfo:
    ...
    project_dir: Optional[str]  # source webagent_tui/ project dir
