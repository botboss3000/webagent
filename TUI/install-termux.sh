...
#             && bash ~/webagent/webagent_tui/install-termux.sh
#
...
if [ -z "${PREFIX:-}" ] || ! command -v pkg >/dev/null 2>&1; then
  die "This installer is for Termux on Android (no 'pkg'/\$PREFIX found). On desktop use webagent_tui/run.bat or 'pip install -e .'."
fi
...
if [ -f "$SRC" ]; then
  ...
  if [ -n "$CAND" ] && [ -f "$CAND/run.py" ] && [ -d "$CAND/webagent_tui" ]; then
    REPO="$CAND"
  fi
fi
if [ -z "$REPO" ]; then
  if [ -f "$TARGET_DEFAULT/run.py" ] && [ -d "$TARGET_DEFAULT/webagent_tui" ]; then
    REPO="$TARGET_DEFAULT"
...
TUI_DIR="$REPO/webagent_tui"
[ -d "$TUI_DIR" ] || die "No webagent_tui/ found in $REPO."
...
export WEBAGENT_TUI_PROJECT="$REPO"
...
# ── 6. API-key check ─────────────────────────────────────────────────────────
HAVE_KEY=0
[ -n "${LLM_API_KEY:-}${OPENROUTER_API_KEY:-}${WEBAGENT_TUI_API_KEY:-}" ] && HAVE_KEY=1
