...
def data_dir() -> Path:
    ...
    By default it lives **right in the project folder** (the directory that holds
    the ``webagent`` package — i.e. the ``TUI`` folder), so the install is fully
    self-contained: the DB, config, monitor/alarm files, and per-subagent session
    history all sit next to the code, and deleting that one folder removes
    everything. Override with ``WEBAGENT_DATA_DIR``.
    ...
    # Running from source: the project folder (config.py → webagent/ → project root).
...
def resolve_provider(project_dir: Optional[Path], saved: "TuiConfig") -> ProviderConfig:
    ...
    * ``provider.json`` above the generic ``LLM_*`` means a **linked repo's key
    ...
    # the manager. Only the WEBAGENT_* env override sits above it. Cleared with
