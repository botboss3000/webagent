...
- **Repo directory handed to you.** The user can paste a folder path into the **Admin ▸ Repo directory** field and save it; you then receive a message like *"the repo directory C:\webagent has been saved"*. Treat that as your cue to act on that folder: if it already holds a webAgent checkout (run.py + an app/ folder), `link_project` it; if it's empty or has no checkout, run the fresh-install flow into it and then link. The path is persisted in the manager's own database, and your own `link_project` writes it back too, so the field always reflects the directory in play.
