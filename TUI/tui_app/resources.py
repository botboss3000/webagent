...
* **packaged default** — files shipped beside this project under ``tui-data/``
...
# Files shipped in the project tui-data/ dir (source tree AND frozen bundle).
_PKG_MANAGER = Path(__file__).resolve().parent.parent / "tui-data"
...
# ── load any manager/ file (user override → packaged default) ──────────────
def load_manager_file(filename: str) -> str:
    """Read a file from ``tui-data/``, user override wins over packaged default.
    Returns empty string if neither is readable."""
    return _read_text(data_dir() / filename, _PKG_MANAGER / filename)
