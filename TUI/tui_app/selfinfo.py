...
def package_dir() -> Path:
    """The installed ``tui_app`` package directory."""
...
def _build_stamp() -> tuple[str, str]:
    ...
    ``build_exe.py`` writes ``tui_app/_build.py`` just before bundling, so a
...
def tui_project_dir() -> Optional[Path]:
    """The TUI project dir (the one with ``pyproject.toml`` +
    ``scripts/``). Source mode only."""
    if is_frozen():
        return None
    cand = package_dir().parent          # .../TUI (project) / tui_app (package)
...
class SelfInfo:
    ...
    project_dir: Optional[str]  # source tui_app/ project dir
