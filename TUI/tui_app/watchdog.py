...
class Watchdog:
    ...
    async def _attempt_restart(self, cfg: dict[str, Any]) -> bool:
        ...
        if len(self._restart_times) >= cap:
            ...
            if self._pb is not None:
                ...
                try:
                    ...
                    pass
            self._inject_restart_event(
                cfg,
                "crash-loop guard paused auto-restart — the server has been restarted "
                f"{len(self._restart_times)}× in the last hour and keeps dying. "
                "Read the diagnostics and server logs, find the root cause, fix it, "
        ...
        # After ANY restart (successful or not), wake the agent so it can
        # investigate why the server died and (if it's still down) bring it
    ...
    async def _check_port(self, cfg: dict[str, Any], root: Path, health: str) -> None:
        ...
        Also **auto-adopts** an untracked-but-healthy server: if something is
        answering /health but the tracked PID is dead, find the real listener and
        adopt it so the manager owns the lifecycle. This fixes the case where the
        manager restarts but a previous (orphaned) server is still serving."""
        from .tools.server import (_pid_alive, _read_pidinfo, _write_pidinfo,
                                   _find_listener_pid)  # lazy: avoid import cycle

        info = _read_pidinfo()
        tracked_pid = int(info["pid"]) if (info and info.get("pid")) else 0
        tracked_alive = bool(tracked_pid and _pid_alive(tracked_pid))

        if health == "running":
            if not tracked_alive:
                # Try to adopt the real listener so our lifecycle tracking is correct.
                real_pid = _find_listener_pid(PORT)
                if real_pid and real_pid != os.getpid() and real_pid != tracked_pid:
                    _write_pidinfo({"pid": real_pid, "port": PORT, "project": str(root)})
                    self._seen_running = True  # there IS a live server, so crash should restart
                    if not self._warned_orphan:
                        self._warned_orphan = True
                        await self._notify(
                            cfg, "Adopted untracked server",
                            f"Found server pid {real_pid} on port {PORT} that the manager "
                            "didn't start — adopted it so lifecycle & restart work cleanly.")
                        if self._pb is not None:
                            await self._remediate(
                                cfg, key="untracked_server", label="Untracked server",
                                kind="builtin",
                                trigger={"port": PORT, "health": health, "pid": real_pid})
                        self._maybe_inject(cfg, "Adopted untracked server",
                                           f"Adopted server pid {real_pid} on port "
                                           f"{PORT} that the manager didn't start.")
                elif not self._warned_orphan:
                    self._warned_orphan = True
                    await self._notify(
                        cfg, "Untracked server",
                        "Port 8080 is serving but the manager has no live PID for it — "
                        "it looks like an externally-started instance. Stop/restart may not be clean.")
                    if self._pb is not None:
                        await self._remediate(
                            cfg, key="untracked_server", label="Untracked server",
                            kind="builtin",
                            trigger={"port": PORT, "health": health})
                    self._maybe_inject(cfg, "Untracked server",
                                       "Port 8080 is serving but the manager has no live "
                                       "PID — it looks like an externally-started instance.")
            elif tracked_alive:
        ...
        venv_py = str(root / ".venv" / "Scripts" / "python.exe") if root else ""
        if sysmetrics.port_in_use(PORT, fallback_python=venv_py):
            now = time.monotonic()
            if now - self._port_alert_at > 3600:
                ...
                if self._pb is not None:
                    await self._remediate(cfg, key="port_zombie",
                                          ...
                                          trigger={"port": PORT, "health": health})
                # Always wake the AI so it knows about the zombie — even when the
                # playbook handles it silently with clear_port, the user wants to
                # see these alerts in the Watchdog session transcript.
                self._maybe_inject(cfg, "Port held by a zombie",
                                   f"Port {PORT} is held by a process that isn't "
                                   "answering /health. This has been surfaced and "
                                   "the playbook may already be handling it.")
    ...
    def _inject_restart_event(self, cfg: dict[str, Any], msg: str) -> None:
        """Hand a RESTART event to the agent — regardless of autonomy level, so the
        agent always knows the server was restarted and can investigate why it died.
        This fires every time _attempt_restart runs."""
        ...
        try:
            self._inject_event(f"Server restart: {body}")
        except Exception:
            pass
    ...
    async def _notify(self, cfg: dict[str, Any], title: str, message: str,
        ...
        ch = channels or cfg.get("channels") or []
        await self._notifier.notify(title, message, ch)
