...
// A WebSocket-silence gate means that whenever the WS is actually delivering for
// this session the poll stays dormant, so there's no double-render and ~no
// overhead on the happy (same-process) path. The loop stays alive for the whole
...
function _tick() {
  ...
  if (app.isProcessing) {
    _idleTicks = 0;
    _reconcileOnce(sid);                               // tracking a live turn → poll fast
    return;
  }
...
async function _reconcileOnce(sid) {
  ...
  try {
    const after = (app.lastSessionSeq && app.lastSessionSeq[sid]) || 0;
    window.__chatPollLastAt = new Date().toISOString();

    const token = localStorage.getItem('auth_token');
    let url = apiPath(
      `/api/v1/db/session-tail?db=local.db&session_id=${encodeURIComponent(sid)}`
    ...
    if (run && run.active && run.assistant_interaction_id) {
      ensureStreamingBubbleForActiveTurn(run.assistant_interaction_id);
    }

    // Advance the resume cursor (also used by the WS reconnect handshake).
    if (!app.lastSessionSeq) app.lastSessionSeq = {};
    const floor = run && typeof run.latest_session_seq === 'number' ? run.latest_session_seq : 0;
    app.lastSessionSeq[sid] = Math.max(app.lastSessionSeq[sid] || 0, maxSeq, floor);
    try {
...
function _renderUserRow(msg) {
  ...
  if (mid && app.chatMessages.querySelector(
        `.chat-bubble.user[data-msg-id="${CSS.escape(String(mid))}"]`)) return;
  if (mid) {
    ...
    for (let i = candidates.length - 1; i >= 0; i--) {
      ...
      if (t === cont.trim()) { b.setAttribute('data-msg-id', String(mid)); return; }
    }
  }
  const bubble = (typeof app.addChatBubble === 'function')
    ? app.addChatBubble('user', cont, undefined, undefined, undefined, mid || undefined)
    : addChatBubble('user', cont, undefined, undefined, undefined, mid || undefined);
