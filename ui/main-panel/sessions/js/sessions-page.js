...
let _searchQuery = '';
let _searchMode = 'content';  // 'title' | 'content' — persisted to localStorage
...
let _stored = {};
try {
  _stored = JSON.parse(localStorage.getItem('data_viewer_view') || '{}');
...
function _saveViewState() {
  localStorage.setItem('data_viewer_view', JSON.stringify({
    viewId: _viewId,
...
function _applySort(sessions) {
  ...
  return sessions.slice().sort((a, b) => {
    // Pinned sessions always come first, regardless of the column sort.
...
async function _fetchAll() {
  ...
  return _fetchForConfig(_activeConfig);
}
...
function _renderTable(rows) {
  ...
  if (table) table.classList.toggle('bin-view', _viewId === 'recycle-bin');
  ...
  if (!candidates || candidates.length === 0) {
    ...
    if (empty) {
      ...
      if (q) {
        ...
        if (txt) txt.textContent = _viewId === 'recycle-bin' ? 'Recycling bin is empty' : (_viewId === 'both' ? 'No sessions yet' : 'No ' + label.toLowerCase() + ' yet');
        if (hint) hint.textContent = _viewId === 'recycle-bin' ? 'Recycled items appear here until permanently deleted' : (_viewId === 'both' ? 'Active and recycled sessions will appear here' : 'Nothing to show in ' + label);
      }
...
function _renderCheckCell(row, isSession) {
  ...
  if (row._cross) return '<td class="col-check"></td>';
...
async function _loadAndRender() {
  const tbody = _qs('sessions-table-body');
  if (tbody && tbody.children.length === 0) _renderSkeletonRows();
  try {
    _activeConfig = await loadViewerConfig(_viewId);
  } catch (e) {
    console.warn('[DataViewer] Failed to load config for', _viewId, e);
    return;
  }
...
function _wireDom() {
  ...
  if (refreshBtn) {
    refreshBtn.addEventListener('click', async () => {
      ...
      // would re-read from localStorage).
...
function _wireGutter() {
  ...
  const _tab = document.getElementById('tab-sessions');
