...
// Canvas toolbar — the Canvas tab's footer control row.
//
...
// canvas rendering code in canvas.js. The two talk only through a small `ctx`
// bridge passed to initCanvasToolbar() and a `syncPages()` handle the core calls
// whenever page state changes. To restyle: edit this file + canvas-toolbar.css.
// To reshape the row: edit _buildToolbar() below (it builds the footer DOM into
// the empty #canvas-footer mount in canvas.html).
//
// What's here: a COMPACT chat pill with a left-side (+) attach button, then a
// CHEVRON-ONLY canvas selector (the current canvas is shown highlighted INSIDE
// the popup, no longer spelled out on the trigger) plus the New Canvas (+) and
// Refresh Canvas buttons. Submitting the pill (Enter / send) opens a floating
...
// The bridge to the canvas core (set by initCanvasToolbar).
let _ctx = null;

// One floating chat-widget at a time (opened by the chat pill). See _openChat.
let _canvasChat = null;
// Which agent the open widget is talking to — so switching to a canvas owned by
// a DIFFERENT agent re-opens the widget against the right agent (no leakage).
let _canvasChatAgentId = null;
...
// canvas's owning agent_id into a display name + to fall back to the default
// webAgent. Resolved once via _loadAgents (see _resolveCanvasAgentName/Id).
...
export function initCanvasToolbar(ctx) {
  _ctx = ctx;
  ...
  const newBtn     = document.getElementById('canvas-new-page-btn');
...
// create_canvas event. Always marks the dropdown "loaded" so it isn't left
  ...
  footer.innerHTML =
    '<div id="canvas-page-nav">' +
      '<div id="canvas-page-label">' +
        ...
        '<div class="canvas-pill-col">' +
          '<div id="canvas-chat-preview" class="canvas-chat-preview" style="display:none;"></div>' +
          // The chat pill — opts into the shared single-line pill (.chat-pill
          // .chat-pill-1line, app1.css) + the floating-glass skin (#canvas-prompt-row,
          // index.css); compact-height trim lives in canvas-toolbar.css. The left
          ...
          '<div id="canvas-prompt-row" class="chat-pill chat-pill-1line">' +
            '<button id="canvas-chat-attach" type="button" class="chat-pill-attach" title="Attach files"><i data-lucide="plus"></i></button>' +
            '<textarea id="canvas-chat-input" class="chat-pill-input" rows="1" autocomplete="off" placeholder="Chat about this page…"></textarea>' +
            '<button id="canvas-chat-voice" type="button" class="chat-pill-voice" title="Voice dictation"><i data-lucide="mic"></i></button>' +
            '<button id="canvas-chat-send" type="button" class="chat-pill-send" title="Send message"><i data-lucide="send"></i></button>' +
          '</div>' +
          '<input id="canvas-chat-file" type="file" multiple style="display:none;">' +
        '</div>' +
        ...
        // The selector is now CHEVRON-ONLY — the current canvas shows highlighted
        // inside the popup, not spelled out on the trigger.
        '<div class="canvas-page-controls">' +
          '<div id="canvas-page-dropdown" class="session-dropdown">' +
            '<button id="canvas-page-dropdown-trigger" type="button" class="session-dropdown-trigger" title="Switch canvas">' +
              '<i data-lucide="chevron-down"></i>' +
            '</button>' +
            '<div id="canvas-page-dropdown-menu" class="session-dropdown-menu" hidden></div>' +
          '</div>' +
          '<button id="canvas-new-page-btn" title="New Canvas" class="header-plus-btn"><i data-lucide="plus"></i></button>' +
          '<button id="canvas-refresh-btn" title="Refresh Canvas" class="header-plus-btn"><i data-lucide="refresh-cw"></i></button>' +
        '</div>' +
      '</div>' +
    '</div>';
}
...
function _bindChatPill() {
  const pill     = document.getElementById('canvas-prompt-row');
  const input    = document.getElementById('canvas-chat-input');
  const sendBtn  = document.getElementById('canvas-chat-send');
  const voiceBtn = document.getElementById('canvas-chat-voice');
  const attachBtn = document.getElementById('canvas-chat-attach');
  const fileInput = document.getElementById('canvas-chat-file');
  const previewBar = document.getElementById('canvas-chat-preview');
...
// then clear the pill. The widget tags each message with the canvas handoff so it
...
// wrapped with the canvas handoff tag (via ctx.buildTaggedPrompt) so it carries
...
async function _openChat(initialMessage, attachmentIds) {
  ...
  // Talk to the agent that OWNS this canvas (its agent_id), falling back to the
  // user's webAgent when the canvas isn't linked yet.
  const agentId = await _resolveCanvasAgentId();

  // If a widget is already open but for a DIFFERENT agent (the user switched to a
  // canvas owned by another agent), close it so the message can't go to the wrong
  // agent — a fresh widget opens against the new owner below.
  if (_canvasChat && _canvasChatAgentId && agentId && _canvasChatAgentId !== agentId) {
    try { _canvasChat.close(); } catch (_) {}   // onClose nulls our refs
    _canvasChat = null;
    _canvasChatAgentId = null;
  }
  if (_canvasChat) {
    _canvasChat.restore();
    if (msg || atts.length) _canvasChat.send(msg, atts);
    return;
  }
  const cur = _ctx.getCurrentPage();
  _canvasChatAgentId = agentId;
  _canvasChat = createChatWidget({
    title: cur ? cur.title : 'Canvas',
    iconName: 'sparkle',
    ensureAgent: () => Promise.resolve(agentId),
    transformMessage: (text) => _ctx.buildTaggedPrompt(text),
    initialMessage: msg,
    initialAttachmentIds: atts,
    onClose: () => { _canvasChat = null; _canvasChatAgentId = null; },
  });
  _canvasChat.open();
}
...
function _closePageActionsPopup() {
  const open = document.querySelector('.session-row-actions[data-source="canvas-page"]');
  if (open) open.remove();
}

function _openPageMenu() {
  const menu = document.getElementById('canvas-page-dropdown-menu');
  const dd   = document.getElementById('canvas-page-dropdown');
  if (!menu) return;
  menu.hidden = false;
...
// fallback owner for canvases no agent has rendered into yet.
function _defaultAgent(map) {
  return map ? Object.values(map).find(a => a.template_id === 'default') : null;
}

// Resolve the display name of the agent that OWNS the current canvas (its
// agent_id), falling back to the default webAgent when the canvas has no owner
// yet (e.g. a brand-new user-made canvas). '' when nothing resolves.
async function _resolveCanvasAgentName() {
...
// Resolve the agent id the chat should talk to: the current canvas's owner, or
// the user's webAgent when the canvas isn't linked to an agent yet.
async function _resolveCanvasAgentId() {
  const cur = _ctx.getCurrentPage();
  const map = await _loadAgents();
...
// Called on each page switch (syncPages), so it tracks the current canvas.
async function _setAgentPlaceholder() {
  const input = document.getElementById('canvas-chat-input');
  if (!input) return;
  const name = await _resolveCanvasAgentName();
...
function _openPageRowActions(slug, row) {
  ...
  popup.dataset.source = 'canvas-page';
...
function _bindPageDropdown() {
  ...
  const trigger = document.getElementById('canvas-page-dropdown-trigger');
  const menu    = document.getElementById('canvas-page-dropdown-menu');
  const dd      = document.getElementById('canvas-page-dropdown');
  ...
  menu.addEventListener('click', (e) => {
    ...
    if (kebab) {
      ...
      const existing = document.querySelector('.session-row-actions[data-source="canvas-page"][data-id="' + slug + '"]');
