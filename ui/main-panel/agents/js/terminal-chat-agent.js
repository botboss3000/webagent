...
/** Populate the Terminal Chat card's tab bar and wire it to swap `body` between
 *  the Settings renderer. Defaults to Settings. */
export function mountTerminalChatCardTabs(tabBar, body, agent) {
  ...
  function show(tab) {
    ...
    body.innerHTML = '';
    if (tab === 'settings') renderTerminalChatSettings(body, agent);
  }

  tabBar.innerHTML = '';
  [['settings', 'Settings']].forEach(([key, label]) => {
