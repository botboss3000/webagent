...
// Refreshes the already-built diagram for the current page from its latest node
// states. Returns false (→ caller does a full renderPage) when the diagram isn't
// built for this page, was detached, or the container width changed enough that a
// re-layout is needed. This is the smooth path that runs on every streaming event.
function updatePageLive(idx) {
  const area = document.getElementById('loop-visual-graph-area');
  if (!area) return false;

  // Diagram must already be built for THIS page and still attached
  if (area._lvRenderedIdx !== idx || !area._lvRoot || !area.contains(area._lvRoot)) return false;

  const page = pages[idx];
  if (!page) return false;

  // A width change means node positions move → fall back to a full re-layout
...
function updatePageSummary(idx) {
  ...
  // Reuse one persistent summary element and only rewrite it when the text
  // actually changes — never remove/append, so the scroll container's child
  // list stays stable on every streaming event (no churn, no scroll nudge).
...
function renderRunHistoryList() {
  ...
  // Incremental: when the run count is unchanged, only refresh the active marker
  // and per-row meta in place (avoids wiping the list — and its scroll — each event)
...
function renderNodesList() {
  ...
  // Incremental: the node directory is a fixed set of rows — once built, just
  // refresh each row's state class / dot / meta in place rather than rebuilding
  // the whole list every event (which churned the DOM and reset its scroll).
