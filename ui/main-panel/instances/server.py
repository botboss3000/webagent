...
async def devices(requesting_user_id: str = ""):
    ...
    for d in rows:
        ...
        if is_self:
            seen_self = True
        # Load custom annotations (tunnel URL) for this device
        _dev_meta = await _meta_get(iid or "")
...
async def tunnel_report(body: TunnelReportBody, request: Request):
    ...
    expected = str((store.load_config().get("slave_tokens") or {}).get(str(port)) or "")
    ...
    if action in ("start", "stop"):
        ...
        while datetime.datetime.now(datetime.timezone.utc) < deadline:
            ...
            if s == "done":
                ...
                return {"ok": True, "job_id": job_id, "queued_for": iid, "result": url}
        ...
        return {"ok": True, "job_id": job_id, "queued_for": iid, "result": ""}
