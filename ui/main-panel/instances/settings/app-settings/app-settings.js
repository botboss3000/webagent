...
// Always-on display toggle uses the Screen Wake Lock API (screen-wake.js).
...
// using the Screen Wake Lock API (ui/shared/js/screen-wake.js).
...
function _initAlwaysOnDisplay() {
  ...
  chk.addEventListener('change', async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      // Activate the wake lock right away — reverts with the checkbox if
      // the save failed.
      if (chk.checked) {
...
// Anonymous session/identity/registration limits in the dedicated
// App Functions "Anonymous Limits" row.
...
function _initAnonSessionLimits() {
  ...
  const identityMaxInput = _qs('ac-anon-identity-max');
  const identityWinInput = _qs('ac-anon-identity-window');
  const globalMaxInput = _qs('ac-anon-global-session-max');
  const globalWinInput = _qs('ac-anon-global-session-window');
  const registrationIpMaxInput = _qs('ac-public-registration-ip-max');
  const registrationGlobalMaxInput = _qs('ac-public-registration-global-max');
  const registrationWinInput = _qs('ac-public-registration-window');
  const saveBtn = _qs('ac-anon-session-save');
  if (!maxInput || !winInput || !identityMaxInput || !identityWinInput || !globalMaxInput || !globalWinInput || !registrationIpMaxInput || !registrationGlobalMaxInput || !registrationWinInput || !saveBtn) return;

  async function _save() {
    if (!isAdmin()) { showRestrictedModal(); return; }
    const vMax = parseInt(maxInput.value, 10);
    const vWin = parseInt(winInput.value, 10);
    const vIdentityMax = parseInt(identityMaxInput.value, 10);
    const vIdentityWin = parseInt(identityWinInput.value, 10);
    const vGlobalMax = parseInt(globalMaxInput.value, 10);
    const vGlobalWin = parseInt(globalWinInput.value, 10);
    const vRegistrationIpMax = parseInt(registrationIpMaxInput.value, 10);
    const vRegistrationGlobalMax = parseInt(registrationGlobalMaxInput.value, 10);
    const vRegistrationWin = parseInt(registrationWinInput.value, 10);
    const anon_session_max = (!isNaN(vMax) && vMax >= 0) ? vMax : 10;
    const anon_session_window = (!isNaN(vWin) && vWin >= 1) ? vWin : 60;
    const anon_identity_max = (!isNaN(vIdentityMax) && vIdentityMax >= 0) ? vIdentityMax : 5;
    const anon_identity_window = (!isNaN(vIdentityWin) && vIdentityWin >= 1) ? vIdentityWin : 86400;
    const anon_global_session_max = (!isNaN(vGlobalMax) && vGlobalMax >= 0) ? vGlobalMax : 25;
    const anon_global_session_window = (!isNaN(vGlobalWin) && vGlobalWin >= 1) ? vGlobalWin : 3600;
    const public_registration_ip_max = (!isNaN(vRegistrationIpMax) && vRegistrationIpMax >= 0) ? vRegistrationIpMax : 5;
    const public_registration_global_max = (!isNaN(vRegistrationGlobalMax) && vRegistrationGlobalMax >= 0) ? vRegistrationGlobalMax : 100;
    const public_registration_window = (!isNaN(vRegistrationWin) && vRegistrationWin >= 1) ? vRegistrationWin : 3600;
    maxInput.value = String(anon_session_max);
    winInput.value = String(anon_session_window);
    identityMaxInput.value = String(anon_identity_max);
    identityWinInput.value = String(anon_identity_window);
    globalMaxInput.value = String(anon_global_session_max);
    globalWinInput.value = String(anon_global_session_window);
    registrationIpMaxInput.value = String(public_registration_ip_max);
    registrationGlobalMaxInput.value = String(public_registration_global_max);
    registrationWinInput.value = String(public_registration_window);
    _markSaving(saveBtn);
    try {
      const res = await _fetch(apiPath('/admin/settings/app'), {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          anon_session_window,
          anon_identity_max,
          anon_identity_window,
          anon_global_session_max,
          anon_global_session_window,
          public_registration_ip_max,
          public_registration_global_max,
  ...
  saveBtn.addEventListener('click', _save);
  [maxInput, winInput, identityMaxInput, identityWinInput, globalMaxInput, globalWinInput, registrationIpMaxInput, registrationGlobalMaxInput, registrationWinInput].forEach(inp => {
...
function _initAnonChatLimits() {
  ...
  const dailyMaxInput = _qs('ac-anon-daily-chat-max');
  const dailyWinInput = _qs('ac-anon-daily-chat-window');
  const globalMaxInput = _qs('ac-anon-global-chat-max');
  const globalWinInput = _qs('ac-anon-global-chat-window');
  const budgetMaxInput = _qs('ac-anon-budget-max');
  const budgetWinInput = _qs('ac-anon-budget-window');
  const saveBtn = _qs('ac-anon-chat-save');
  if (!maxInput || !winInput || !dailyMaxInput || !dailyWinInput || !globalMaxInput || !globalWinInput || !budgetMaxInput || !budgetWinInput || !saveBtn) return;

  async function _save() {
    if (!isAdmin()) { showRestrictedModal(); return; }
    const vMax = parseInt(maxInput.value, 10);
    const vWin = parseInt(winInput.value, 10);
    const vDailyMax = parseInt(dailyMaxInput.value, 10);
    const vDailyWin = parseInt(dailyWinInput.value, 10);
    const vGlobalMax = parseInt(globalMaxInput.value, 10);
    const vGlobalWin = parseInt(globalWinInput.value, 10);
    const vBudgetMax = parseInt(budgetMaxInput.value, 10);
    const vBudgetWin = parseInt(budgetWinInput.value, 10);
    const anon_chat_max = (!isNaN(vMax) && vMax >= 0) ? vMax : 5;
    const anon_chat_window = (!isNaN(vWin) && vWin >= 1) ? vWin : 300;
    const anon_daily_chat_max = (!isNaN(vDailyMax) && vDailyMax >= 0) ? vDailyMax : 10;
    const anon_daily_chat_window = (!isNaN(vDailyWin) && vDailyWin >= 1) ? vDailyWin : 86400;
    const anon_global_chat_max = (!isNaN(vGlobalMax) && vGlobalMax >= 0) ? vGlobalMax : 50;
    const anon_global_chat_window = (!isNaN(vGlobalWin) && vGlobalWin >= 1) ? vGlobalWin : 300;
    const anon_budget_max = (!isNaN(vBudgetMax) && vBudgetMax >= 0) ? vBudgetMax : 100;
    const anon_budget_window = (!isNaN(vBudgetWin) && vBudgetWin >= 1) ? vBudgetWin : 86400;
    maxInput.value = String(anon_chat_max);
    winInput.value = String(anon_chat_window);
    dailyMaxInput.value = String(anon_daily_chat_max);
    dailyWinInput.value = String(anon_daily_chat_window);
    globalMaxInput.value = String(anon_global_chat_max);
    globalWinInput.value = String(anon_global_chat_window);
    budgetMaxInput.value = String(anon_budget_max);
    budgetWinInput.value = String(anon_budget_window);
    _markSaving(saveBtn);
    try {
      const res = await _fetch(apiPath('/admin/settings/app'), {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          anon_chat_window,
          anon_daily_chat_max,
          anon_daily_chat_window,
          anon_global_chat_max,
          anon_global_chat_window,
          anon_budget_max,
          anon_budget_window,
        }),
      });
  ...
  saveBtn.addEventListener('click', _save);
  [maxInput, winInput, dailyMaxInput, dailyWinInput, globalMaxInput, globalWinInput, budgetMaxInput, budgetWinInput].forEach(inp => {
    inp.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') { e.preventDefault(); _save(); }
    });
  });
...
function _initAnonChatIpLimits() {
  ...
  async function _save() {
    ...
    const vMax = parseInt(maxInput.value, 10);
    const vWin = parseInt(winInput.value, 10);
    const anon_chat_ip_max = (!isNaN(vMax) && vMax >= 0) ? vMax : 10;
...
async function _loadAppSettings(data) {
  try {
    ...
    const aiMax = _qs('ac-anon-identity-max');
    if (aiMax) {
      const v = parseInt(data.anon_identity_max, 10);
      aiMax.value = (!isNaN(v) && v >= 0) ? v : 5;
    }
    const aiWin = _qs('ac-anon-identity-window');
    if (aiWin) {
      const v = parseInt(data.anon_identity_window, 10);
      aiWin.value = (!isNaN(v) && v >= 1) ? v : 86400;
    }
    const agsMax = _qs('ac-anon-global-session-max');
    if (agsMax) {
      const v = parseInt(data.anon_global_session_max, 10);
      agsMax.value = (!isNaN(v) && v >= 0) ? v : 25;
    }
    const agsWin = _qs('ac-anon-global-session-window');
    if (agsWin) {
      const v = parseInt(data.anon_global_session_window, 10);
      agsWin.value = (!isNaN(v) && v >= 1) ? v : 3600;
    }
    const regIpMax = _qs('ac-public-registration-ip-max');
    if (regIpMax) {
      const v = parseInt(data.public_registration_ip_max, 10);
      regIpMax.value = (!isNaN(v) && v >= 0) ? v : 5;
    }
    const regGlobalMax = _qs('ac-public-registration-global-max');
    if (regGlobalMax) {
      const v = parseInt(data.public_registration_global_max, 10);
      regGlobalMax.value = (!isNaN(v) && v >= 0) ? v : 100;
    }
    const regWin = _qs('ac-public-registration-window');
    if (regWin) {
      const v = parseInt(data.public_registration_window, 10);
      regWin.value = (!isNaN(v) && v >= 1) ? v : 3600;
    }
    const acMax = _qs('ac-anon-chat-max');
    if (acMax) {
      const v = parseInt(data.anon_chat_max, 10);
      acMax.value = (!isNaN(v) && v >= 0) ? v : 5;
    }
    ...
    const adcMax = _qs('ac-anon-daily-chat-max');
    if (adcMax) {
      const v = parseInt(data.anon_daily_chat_max, 10);
      adcMax.value = (!isNaN(v) && v >= 0) ? v : 10;
    }
    const adcWin = _qs('ac-anon-daily-chat-window');
    if (adcWin) {
      const v = parseInt(data.anon_daily_chat_window, 10);
      adcWin.value = (!isNaN(v) && v >= 1) ? v : 86400;
    }
    const agcMax = _qs('ac-anon-global-chat-max');
    if (agcMax) {
      const v = parseInt(data.anon_global_chat_max, 10);
      agcMax.value = (!isNaN(v) && v >= 0) ? v : 50;
    }
    const agcWin = _qs('ac-anon-global-chat-window');
    if (agcWin) {
      const v = parseInt(data.anon_global_chat_window, 10);
      agcWin.value = (!isNaN(v) && v >= 1) ? v : 300;
    }
    const budgetMax = _qs('ac-anon-budget-max');
    if (budgetMax) {
      const v = parseInt(data.anon_budget_max, 10);
      budgetMax.value = (!isNaN(v) && v >= 0) ? v : 100;
    }
    const budgetWin = _qs('ac-anon-budget-window');
    if (budgetWin) {
      const v = parseInt(data.anon_budget_window, 10);
      budgetWin.value = (!isNaN(v) && v >= 1) ? v : 86400;
    }
    const aciMax = _qs('ac-anon-chat-ip-max');
    if (aciMax) {
      const v = parseInt(data.anon_chat_ip_max, 10);
      aciMax.value = (!isNaN(v) && v >= 0) ? v : 10;
    }
    ...
    console.warn('settings: could not load app settings', e);
  }
}
...
async function _loadAppearance(data, bgData) {
  ...
  try {
    ...
    console.warn('settings: appearance values load failed', e);
  }
  ...
  try {
    ...
    console.warn('settings: background catalog load failed', e);
  }
