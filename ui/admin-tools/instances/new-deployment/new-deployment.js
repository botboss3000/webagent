...
async function _ensureDeploy() {
  ...
  _deployLoaded = true;
  try {
    const m = await import('../../app-config/data-settings/deploy.js');
    m.initDeploy();
...
async function _ensureDns() {
  ...
  try {
    ...
    const m = await import('../../app-config/data-settings/dns.js');
    m.initDns();
...
function _wireRows(root) {
  ...
  ['ac-deploy-repo-row', 'ac-deploy-config-row', 'ac-dns-main-row'].forEach((id) => {
