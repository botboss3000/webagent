...
const state = {
  ...
  lastApply: null,     // last /apply payload (for the conflict hand-off)
};
...
function _renderConfig(s) {
  const url = _qs('upd-url');
  const branch = _qs('upd-branch');
  const hint = _qs('upd-mode-hint');
  const initBtn = _qs('upd-init');
  ...
  if (hint) {
    if (!s.is_git) {
      hint.textContent = 'Not a git repository — initialize git to enable selective updates.';
      hint.className = 'upd-hint upd-hint-warn';
    } else if (!s.has_upstream) {
      hint.textContent = 'Git repo detected. Save an upstream URL, then Fetch.';
      hint.className = 'upd-hint';
    } else {
      hint.textContent = 'Git mode — ' + (s.upstream_url || '') +
        (s.last_synced_commit ? (' · last synced ' + s.last_synced_commit.slice(0, 7)) : '');
      hint.className = 'upd-hint';
    }
  }
...
function _renderFolders(s) {
  const host = _qs('upd-folders');
  if (!host) return;
  if (!s.is_git) {
    host.innerHTML = '<div class="upd-empty">Initialize git first to choose folders.</div>';
    return;
  }
  const folders = s.folders || [];
  if (!folders.length) {
    host.innerHTML = '<div class="upd-empty">' +
      (s.has_upstream ? 'No syncable folders found.' : 'Save an upstream repo, then Fetch.') +
      '</div>';
    return;
  }
  host.innerHTML = '';
  for (const f of folders) {
    ...
    const changed = f.changed_files > 0
      ? '<span class="upd-badge">' + f.changed_files + '</span>'
      : '';
    row.innerHTML =
      '<input type="checkbox" class="upd-folder-cb" data-path="' + _esc(f.path) + '"' +
      (f.tracked ? ' checked' : '') + ' />' +
      '<span class="upd-folder-name">' + _esc(f.path) + '</span>' +
      (f.present_locally ? '' : '<span class="upd-folder-tag">not local</span>') +
      changed;
...
function _renderError(msg) {
  const host = _qs('upd-folders');
  if (host) host.innerHTML = '<div class="upd-empty upd-hint-warn">' + _esc(msg) + '</div>';
}

function _trackedFromUi() {
  return Array.from(document.querySelectorAll('.upd-folder-cb'))
...
function _wireSidebar() {
  ...
  if (save) save.addEventListener('click', async () => {
    ...
    try {
      await _api('/config', {
        ...
        body: JSON.stringify({
          upstream_url: (_qs('upd-url') || {}).value || '',
          upstream_branch: (_qs('upd-branch') || {}).value || 'main',
          tracked_paths: _trackedFromUi(),
        }),
      });
      _toast('Saved.');
      await refreshStatus();
  ...
  if (init) init.addEventListener('click', async () => {
    ...
    try {
      ...
      _toast('Initialized.');
      await refreshStatus();
    } catch (e) { _toast('Init failed: ' + e.message); }
...
function _wireMain() {
  ...
  const apply = _qs('upd-apply');
  if (apply) apply.addEventListener('click', () => doApply());
  const body = _qs('upd-main-body');
  if (body) body.addEventListener('click', _onMainClick);
}
...
function _renderMainFromStatus() {
  ...
  if (!s || !s.has_upstream) {
    body.innerHTML = '<div class="upd-hint upd-main-hint">Configure an upstream repo and Fetch to see changes.</div>';
    if (apply) apply.disabled = true;
    return;
  }
  if (!folders.length) {
    body.innerHTML = '<div class="upd-hint upd-main-hint">No changes in your tracked folders. You’re up to date.</div>';
    if (apply) apply.disabled = true;
    return;
  }
...
async function _onMainClick(e) {
  ...
  if (diffBtn) {
    ...
    const pre = document.querySelector('.upd-diff[data-path="' + CSS.escape(path) + '"]');
    if (!pre) return;
    if (!pre.hidden) { pre.hidden = true; return; }
    pre.hidden = false;
    pre.textContent = 'Loading diff…';
    try {
      const r = await _api('/diff?path=' + encodeURIComponent(path));
      pre.textContent = (r.diff || '(no diff)') + (r.truncated ? '\n… (truncated)' : '');
...
async function doApply() {
  const paths = Array.from(document.querySelectorAll('.upd-apply-cb'))
  ...
  _toast('Applying…');
  try {
    const r = await _api('/apply', {
      method: 'POST', headers: _jsonHeaders(),
      body: JSON.stringify({ paths, create_backup: true }),
    });
    state.lastApply = r;
    _renderApplyResult(r);
    await refreshStatus();
  } catch (e) {
    _toast('Apply failed: ' + e.message);
    if (apply) apply.disabled = false;
  }
}
...
function _renderApplyResult(r) {
  ...
  const conflicts = r.conflict_files || [];
  const lines = [
    ['Merged cleanly', n('clean')],
    ['Added', n('added')],
    ['Binary replaced', n('binary')],
    ['Upstream-deleted (kept local)', n('deleted-upstream')],
    ['Conflicts', conflicts.length],
    ['Errors', n('error')],
  ].filter(([, c]) => c > 0)
    .map(([l, c]) => '<li>' + _esc(l) + ': <strong>' + c + '</strong></li>').join('');

  let html = '<div class="upd-result">' +
    '<div class="upd-result-title">Update applied' +
  ...
  if (conflicts.length) {
    html += '<div class="upd-conflict-block">' +
      '<div class="upd-conflict-title">' + conflicts.length +
        ' file(s) have merge conflicts (markers written in place):</div>' +
      '<ul class="upd-conflict-list">' +
        conflicts.slice(0, 50).map((f) => '<li><code>' + _esc(f) + '</code></li>').join('') +
        (conflicts.length > 50 ? '<li>… and ' + (conflicts.length - 50) + ' more</li>' : '') +
      '</ul>' +
      ...
      '<div class="upd-hint">Or edit them in File Manager / Source Control. Conflicts use standard ' +
        '<code>&lt;&lt;&lt;&lt;&lt;&lt;&lt;</code> markers.</div>' +
    '</div>';
  }

  if (r.backend_changed) {
    html += '<div class="upd-hint upd-hint-warn">Backend (.py) files changed — restart the server to load them ' +
      '(Source Control → restart, or restart the process).</div>';
  }
  html += '</div>';
  body.innerHTML = html;
...
// Hand the conflict list to the web agent by seeding the chat composer with a
// ready-to-send prompt. git_control's resolve_conflict / edit_source /
// commit_and_push do the actual work once the admin reviews & sends.
function _handToAgent() {
  const r = state.lastApply;
  const conflicts = (r && r.conflict_files) || [];
