...
async function _loadRuns() {
  ...
  try {
    ...
    tbody.innerHTML = `<tr><td colspan="9" class="ac-table-empty" style="color:var(--danger);">Error loading runs</td></tr>`;
  }
}
...
function _loadingDetail() {
  return `<div class="ac-opt-detail-loading"><i data-lucide="loader-2" class="lucide-spin" style="width:14px;height:14px;"></i> Loading stages…</div>`;
}
...
async function _loadDetail(runId) {
  try {
    ...
    _stageCache[runId] = { stages: [], html: `<div class="ac-opt-detail-loading" style="color:var(--danger);">Failed to load stages</div>` };
  }
...
function _renderStages(stages) {
  ...
  for (const s of stages) {
    ...
    const action = (s.openable && s.session_id)
      ? `<button type="button" class="ac-btn ac-btn-ghost ac-opt-open-btn" data-open-session="${_esc(s.session_id)}"><i data-lucide="message-square" style="width:13px;height:13px;"></i> Open</button>`
