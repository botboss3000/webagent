...
function bindMount(prefix) {
  const m = {
    ...
    canvasBadge: qs(`${prefix}canvas-badge`),
    canvasMode: qs(`${prefix}canvas-mode`),
    canvasNotes: qs(`${prefix}canvas-notes`),
    canvasSave: qs(`${prefix}canvas-save`),
    canvasOutput: qs(`${prefix}canvas-output`),
    ...
    fulldbBadge: qs(`${prefix}fulldb-badge`),
    fulldbWarning: qs(`${prefix}fulldb-warning`),
    fulldbNotes: qs(`${prefix}fulldb-notes`),
...
const CANVAS_NOTES = {
  filesystem: notesHtml(
    ['Zero setup — canvases are <code>.html</code> files in <code>visuals/users/</code>.', 'Inspectable on disk, easy to hand-edit, git-commit, or back up.', 'Fastest serving path (static file read).'],
    ['Ephemeral on stateless cloud deploys: canvases are wiped on container restart unless a persistent volume is mounted.', 'Not queryable — no search across canvases by content.'],
    'Default. Best for local-hosted use, single-server deploys, or anywhere the disk is persistent.'
  ),
  database: notesHtml(
    ['Full HTML stored in the <code>canvases</code> table — survives container restarts everywhere.', 'Backed up with the rest of the DB; no separate file backup needed.', 'Queryable: SQL across canvas content and metadata.'],
...
function applyState(m, state) {
  ...
  if (m.canvasMode) {
    m.canvasMode.value = pgs.mode || 'filesystem';
    renderNotes(m.canvasNotes, CANVAS_NOTES, m.canvasMode.value);
    if (m.canvasBadge) m.canvasBadge.textContent = `active: ${pgs.mode || 'filesystem'}`;
  }
  const lock = !!state.env_locked;
  for (const btn of [m.btnSave, m.btnActivate, m.secretsSave, m.canvasSave, m.encSave, m.encKekGen, m.encKekRotate, m.encMigrate, m.encDecryptAll]) {
...
// ── Full-database (whole-file) encryption — per-database toggles ──────────────
async function loadFullDb(m) {
  ...
  if (!res.ok) {
    if (m.fulldbBadge) m.fulldbBadge.textContent = `error ${res.status}`;
    return;
  }
  const s = await res.json();
  const onCount = (s.databases || []).filter(d => d.enabled).length;
  if (m.fulldbBadge) {
    m.fulldbBadge.textContent = !s.sqlcipher_available
      ? 'engine not installed'
      : (onCount ? `on: ${onCount}/${(s.databases || []).length}` : 'all plaintext');
  }
  ...
  for (const d of (s.databases || [])) {
    const row = document.createElement('div');
    row.style.cssText = 'display:flex;align-items:center;gap:10px;padding:6px 8px;border:var(--border-width) solid var(--border);border-radius:6px;';
    const label = document.createElement('div');
    label.style.cssText = 'flex:1;min-width:0;';
    const diskState = d.encrypted_on_disk
      ? '<span style="color:var(--success);">encrypted on disk</span>'
      : (d.exists ? '<span style="opacity:0.6;">plaintext on disk</span>' : '<span style="opacity:0.5;">not created yet</span>');
    const pending = (d.enabled !== d.encrypted_on_disk)
      ? ' <span style="color:var(--warning);">· pending restart</span>' : '';
    label.innerHTML = `<div style="font-size:12px;font-weight:600;">${esc(d.label || d.id)}</div>`
      + `<div style="font-size:10px;margin-top:1px;">${diskState}${pending}</div>`;
    const toggle = document.createElement('input');
    toggle.type = 'checkbox';
    toggle.checked = !!d.enabled;
    toggle.disabled = !canToggle;
    toggle.style.cssText = 'width:16px;height:16px;cursor:' + (canToggle ? 'pointer' : 'not-allowed') + ';flex:0 0 auto;';
    toggle.title = canToggle ? 'Encrypt this database at rest' : (s.reason || 'Cannot enable here');
    toggle.addEventListener('change', async () => {
      const want = toggle.checked;
      const verb = want ? 'Encrypt' : 'Decrypt';
      if (!confirm(`${verb} the "${d.label || d.id}" database at rest? It will be converted (backup-first) on the next server restart.`)) {
        toggle.checked = !want;
        return;
      }
      out(m.fulldbOutput, `${verb}ing "${d.id}" — saving intent…`, null);
      const r = await call('/admin/storage/encryption/full-db/set', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ requesting_user_id: uid(), db_id: d.id, enabled: want }),
      });
      const okFlag = !!(r.body && r.body.ok);
      out(m.fulldbOutput, okFlag
        ? `${r.body.message} (database: ${d.id}) — restart the server to apply.`
        : (r.body || { error: 'no response' }), okFlag);
      if (!okFlag) toggle.checked = !want;
      loadFullDb(m);
    });
    row.appendChild(label);
...
function wire(m) {
  ...
  if (m.canvasMode) {
    m.canvasMode.addEventListener('change', () => {
      renderNotes(m.canvasNotes, CANVAS_NOTES, m.canvasMode.value);
    });
  }
  ...
  m.canvasSave && m.canvasSave.addEventListener('click', async () => {
    if (!confirm(`Switch canvas storage to "${m.canvasMode.value}"?`)) return;
    const r = await call('/admin/storage/canvases/mode', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ requesting_user_id: uid(), mode: m.canvasMode.value }),
    });
    out(m.canvasOutput, r.body || { error: 'no response' }, !!(r.body && r.body.ok));
    if (r.body && r.body.ok && m.canvasBadge) {
      m.canvasBadge.textContent = `active: ${m.canvasMode.value}`;
    }
  });
