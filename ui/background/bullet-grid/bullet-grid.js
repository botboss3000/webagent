...
(function () {
  ...
  function frame(t) {
    ...
    for (var i = 0; i < dots.length; i++) {
      ...
      var dx = homeX - pointerX, dy = homeY - pointerY;
      var dist = Math.sqrt(dx * dx + dy * dy);
      var influence = 0;
      var targetX = homeX, targetY = homeY;
      if (dist < RADIUS && dist > 0.01) {
        // Smooth (eased) falloff so the edge of the radius isn't a hard line.
        influence = 1 - dist / RADIUS;
        influence = influence * influence;
        var off = influence * PUSH;
        targetX = homeX + (dx / dist) * off;
        targetY = homeY + (dy / dist) * off;
      }
  ...
  function onPointerLeave() { pointerX = -9999; pointerY = -9999; }
