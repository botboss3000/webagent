...
import { switchToSession, loadingSpinnerMarkup } from './session-core.js';
...
function _initPinSwipeNavigation() {
  ...
  function _sessionsInOrder() {
    ...
    // unpinned (by activity, descending). Hidden sessions are excluded so nav
  ...
  // for the loading spinner, then crossfade the panel away so the spinner
  // fades in beneath it. Only then switch sessions — the loaded transcript
  // fades in on top of the spinner (see _renderSessionWindowed).
  function _commitSwipe(dir) {
    ...
    // after the 400ms slide). Swap its content for the spinner BEFORE bringing
    ...
    inner.innerHTML = loadingSpinnerMarkup();
  ...
  wrapper.addEventListener('pointercancel', (e) => { try { wrapper.releasePointerCapture(e.pointerId); } catch (_) {} _active = false; _resetOffset(false); });
