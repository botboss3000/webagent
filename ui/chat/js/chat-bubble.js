...
function _bubbleGroupable(bubble) {
  ...
  // Only plain agent prose joins — tool pills, errors, and deleted stand alone.
...
function addChatBubble(role, text, extraClass, imageUrl, turnId, msgId, createdAt) {
  ...
  // Error / interrupted / placeholder bubbles do NOT participate in merging.
  if (role === 'user') {
  ...
  if (role === 'agent' && !isToolOnly && extraClass !== 'error' && extraClass !== 'interrupted') {
