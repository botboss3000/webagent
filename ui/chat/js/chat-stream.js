...
function showScoutResponse(content, previewId, ownerTurnId, createdAt, phase) {
  if (!app.chatMessages) return null;
  const owner = String(ownerTurnId || '');
  if (phase === 'starting' && owner) {
    app.chatMessages.querySelectorAll('.chat-bubble.agent.scout-preview').forEach(candidate => {
      if ((candidate.dataset.scoutOwner || '') === owner) return;
  ...
  const selector = owner
    ? `.chat-bubble.agent.scout-preview[data-scout-owner="${CSS.escape(owner)}"]`
    : '.chat-bubble.agent.scout-preview';
  let bubble = app.chatMessages.querySelector(selector);
  if (phase === 'end' || !(content || '').trim()) {
    if (bubble) {
  ...
  if (!bubble) {
    bubble = addChatBubble(
      'agent', content, 'scout-preview', undefined,
      previewId || undefined, previewId || undefined, createdAt,
    );
    const label = bubble.querySelector(':scope > .label');
    if (label) label.textContent = 'Scout · provisional';
  } else {
    _setBubbleText(bubble, content, 'scout-preview');
  }
...
function dismissScoutResponse(ownerTurnId) {
  return showScoutResponse('', '', ownerTurnId, undefined, 'end');
}
...
app.showScoutResponse = showScoutResponse;
app.dismissScoutResponse = dismissScoutResponse;
