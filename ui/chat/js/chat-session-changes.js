...
function _render() {
  ...
  const count = document.getElementById('chat-changes-count');
  ...
  if (count) { count.textContent = String(_current.length); count.hidden = !_current.length; }
...
export async function initSessionChanges() {
  const toggle = document.getElementById('chat-changes-toggle');
  ...
  toggle.hidden = false;
  toggle.addEventListener('click', () => { _visible = !_visible; panel.hidden = !_visible; if (_visible) refreshSessionChanges(); });
