...
// reconnect.js. When unreachable the session-dropdown chevron shows a spinner
// and ignores clicks.
let _serverReachable = true;

// Health-poll hook (reconnect.js): toggle the session-dropdown between normal
// and offline-spinner states.
...
function _renderMessages(messages, sessionId, run, prepend) {
  ...
  for (const msg of messages) {
    if (msg.role === 'user') {
      app.addChatBubble('user', msg.content, undefined, undefined, undefined, msg.id, msg.created_at);
      count++;
      ...
      if (msg.status === 'streaming') {
        ...
        app.addChatBubble('agent', text, 'error', undefined, msg.id, msg.created_at);
        count++;
      } else {
        app.addChatBubble('agent', text, undefined, undefined, msg.id, msg.created_at);
        count++;
      }
    }
  }
...
function _recyclePlaceholderToBubble(placeholder, msgId) {
  ...
  // Store creation timestamp on the bubble so _addBubbleActions renders it
  {
  ...
  // Add bubble actions (includes timestamp via _addBubbleActions)
...
export async function loadSessionChat(sessionId) {
  ...
  try {
    ...
    if (cached && (now - cached.loadedAt) < _CACHE_TTL_MS) {
      ...
      // Show live context from cached messages
    ...
    // Show live context from loaded messages
    if (typeof app.setContextFromMessages === 'function') {
...
export function _cacheAppendMessage(sessionId, msg) {
  ...
  // Update ctx indicator after every new message
  if (typeof app.setContextFromMessages === 'function') {
    app.setContextFromMessages(cached.messages);
  }
}
...
export function initSessions() {
  ...
  // ── Swipe navigation between pinned sessions ──
  // Horizontal swipe on the transcript area: the current session's bubbles
  // slide out and the next pinned session's actual bubbles slide in.
  (function _initPinSwipeNavigation() {
    const inner = app.chatMessages;            // #chat-messages-inner (scroller)
    const wrapper = inner && inner.parentElement; // #chat-messages (position: relative)
    if (!inner || !wrapper) return;

    let _startX = 0, _startY = 0, _startT = 0;
    let _active = false;
    let _stashedDirection = 0;
    let _nextPanel = null; // a sibling panel sliding in from the side
    let _pinnedPreloaded = false;
    ...
    function _preloadAllPinned() {
      if (_pinnedPreloaded) return;
      _pinnedPreloaded = true;
      for (const s of _pinnedInOrder()) {
        if (s.id !== app.currentSessionId) {
          _fetchMessages(s.id, 20).then(data => {
            if (!data.restricted && data.messages) {
              _messageCache.set(s.id, { messages: data.messages, loadedAt: Date.now() });
            }
          }).catch(() => {});
        }
      }
    ...
    /** Render cached messages into a container as proper chat bubbles. */
    function _renderBubbles(sid, container) {
      ...
      container.innerHTML = '';
      for (const msg of cached.messages) {
        // Skip tool-call rows (they have a tool_name) and tool rows
        if (msg.tool_name || msg.role === 'tool') continue;
        // Only render user and assistant messages
        if (msg.role !== 'user' && msg.role !== 'assistant') continue;
        let text = (msg.content || '').trim();
        // Strip tool-call appendix from assistant content
        const toolCallIdx = text.indexOf('\n\n[Tool calls: ');
        if (toolCallIdx !== -1) text = text.slice(0, toolCallIdx);
        if (!text) continue;
        const bubble = document.createElement('div');
        ...
        // ── Relative timestamp (peek-preview panel) ──
        {
          const createdAtMs = msg.created_at ? new Date(msg.created_at.replace(' ', 'T') + 'Z').getTime() : Date.now();
    ...
    function _ensureNextPanel(direction) {
      ...
      // Position: starts off-screen left (-100%) or right (+100%)
    ...
    function _applyOffset(dx) {
      const dir = dx > 0 ? -1 : 1; // right → prev (-1), left → next (1)
      ...
      // Convert px drag to fraction of wrapper width
      const w = wrapper.clientWidth || 380;
      const frac = Math.max(-1, Math.min(1, dx / w)); // -1..1

      // Slide the current inner in pixels (as the user drags)
      inner.style.transition = 'none';
      inner.style.transform = 'translateX(' + Math.round(dx) + 'px)';
      inner.style.opacity = String(Math.max(0.3, 1 - Math.abs(frac)));

      // Slide the next panel from the opposite side (in %)
      if (_nextPanel) {
    ...
    setTimeout(_preloadAllPinned, 2000);
    ...
    wrapper.addEventListener('pointerup', (e) => {
      ...
      if (dt < 500 && Math.abs(dx) > 20) {
        // Snap to next instantly before switching
        _resetOffset(true);
        _go(dx > 0 ? -1 : 1);
      } else {
        _resetOffset(false);
    ...
    // ── Desktop nav arrows (prev/next pinned session) ──
