...
// Mounted into the Agent Config tab by ui/js/agents.js. Renders:
...
function _btn(label, className = 'agents-btn') {
...
// ── Public API: mount() ───────────────────────────────────────────────────────
// Renders a collapsible card matching the existing LLM card pattern in agents.js.
// Body is collapsed by default; chevron + badge ("N attached") in the header.
export async function mount(host, agent, userId) {
  ...
  addBox.style.cssText =
    'margin-top:6px;padding:10px;border:1px dashed var(--border,#2a2a3a);' +
    'border-radius:6px;background:var(--bg-1,#1a1b26);';
...
function _renderAttachmentRow(att, agent, userId, onChanged) {
  ...
  row.style.cssText =
    'border:1px solid var(--border,#2a2a3a);border-radius:6px;padding:10px;' +
    'background:var(--bg-1,#1a1b26);display:flex;flex-direction:column;gap:6px;';
  ...
  typeTag.style.cssText =
    'font-size:11px;padding:2px 6px;border-radius:4px;background:var(--bg-2,#222433);' +
    'color:var(--fg-2,#a9b1d6);';
  typeTag.textContent = att.type || '?';
  const statusTag = document.createElement('span');
  statusTag.style.cssText =
    'font-size:11px;padding:2px 6px;border-radius:4px;' +
    `background:${att.status === 'active' ? '#1e3a2e' : att.status === 'error' ? '#3a1e1e' : '#2a2a3a'};` +
    `color:${att.status === 'active' ? '#9ece6a' : att.status === 'error' ? '#f7768e' : '#a9b1d6'};`;
...
function _wrapField(label, input) {
  ...
  lab.style.cssText = 'font-size:11px;color:var(--fg-2,#a9b1d6);';
  w.appendChild(lab);
